...
function dl(attrs, children) {
  return element2("dl", attrs, children);
}
...
// build/dev/javascript/vn_provinces_demo/router.mjs
var Home = class extends CustomType {
};
var Province = class extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
function parse_to_route(query) {
  let query$1 = reverse(query);
  let _block;
  let _pipe = query$1;
  _block = find_map(
    _pipe,
  ...
  return dl(
    ...
    toList([
      ...
      dt(
        toList([class$("flex")]),
        toList([
          span(toList([class$("block")]), toList([text3("M\xE3 s\u1ED1:")])),
          span(
            toList([class$("block flex-grow text-end")]),
            toList([text3(to_string(province.code))])
          )
        ])
      )
    ])
    ...
    toList([
      ...
      dt(
        ...
        toList([
          ...
          span(
            toList([class$("block flex-grow text-end")]),
            toList([text3(to_string(ward.code))])
          )
        ])
      )
    ])
...
var Model = class extends CustomType {
  constructor(route, provinces, province_combobox_state, wards, ward_combobox_shown, ward_filter_text, filtered_wards, selected_ward) {
    super();
    this.route = route;
    this.provinces = provinces;
    this.province_combobox_state = province_combobox_state;
    this.wards = wards;
...
function view2(model) {
  ...
  echo(selected_province, void 0, "src/vn_provinces_demo.gleam", 268);
  echo(selected_ward, void 0, "src/vn_provinces_demo.gleam", 269);
  let _block;
...
function on_url_change(uri) {
  ...
  let _pipe$1 = map(
    _pipe,
    (q) => {
      return from_result(parse_query(q));
    }
  );
  let _pipe$2 = flatten(_pipe$1);
  let _pipe$3 = unwrap(_pipe$2, toList([]));
  _block = parse_to_route(_pipe$3);
  let route = _block;
...
function update2(model, msg) {
  if (msg instanceof ProvinceSelected) {
    ...
    echo(p, void 0, "src/vn_provinces_demo.gleam", 158);
    let model$1 = new Model(
      model.route,
      model.provinces,
      (() => {
    ...
    echo(w, void 0, "src/vn_provinces_demo.gleam", 232);
    let model$1 = new Model(
      model.route,
      model.provinces,
      model.province_combobox_state,
    ...
    if ($ instanceof Ok) {
      ...
      echo(provinces, void 0, "src/vn_provinces_demo.gleam", 82);
      let model$1 = new Model(
        model.route,
        model.provinces,
        (() => {
    ...
    if ($ instanceof Ok) {
      ...
      let e = $[0];
      echo(e, void 0, "src/vn_provinces_demo.gleam", 119);
      return [model, none2()];
    }
    ...
    if (new_route instanceof Home) {
      ...
      let p = new_route[0];
      echo(model, void 0, "src/vn_provinces_demo.gleam", 140);
